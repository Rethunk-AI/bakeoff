...
bench/
  ...
  worker.py          opt-in distributed pull client (claim / heartbeat / submit)
...
- **Worker mode is opt-in.** `bench.worker` polls a bakeoff-results queue; it must not be wired into the default `bench.runner` matrix loop. Empty queue sleeps. `--models` on the runner is the execute seam so a claimed job still loads one model at a time.
