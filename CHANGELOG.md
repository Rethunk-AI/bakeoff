...
- **Worker fail + pause handling** — a runner crash POSTs `/api/queue/{id}/fail` so the queue can retry instead of leaving the claim stuck until TTL. A paused runner (403) sleeps like an empty queue rather than exiting. Submitted JSON is matched to the claimed `model_id` and must be newer than execute start; a missing match fails the job instead of shipping a stub or the previous run.
